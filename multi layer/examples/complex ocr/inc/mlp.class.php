	...
	class MLP {
		...
		public function work(array $input) {
			...
			for ($item = 0; $item < $this->layers[0]; $item++) {
				$this->values[0][$item] = floatval($input[$item]);
				$this->output[0][$item] = floatval($input[$item]);
			}
		...
		public function backpropagation(array $test) {
			...
			if ($result != $test['out']) {
				...
				for ($item = 0; $item < $this->layers[($this->num_layers - 1)]; $item++) {
					...
					$this->deltas[($this->num_layers - 1)][$item] = round(((floatval($test['out'][$item]) - $result[$item]) * self::derivate($this->values[($this->num_layers - 1)][$item])), 4);
					$error += pow((floatval($test['out'][$item]) - $result[$item]), 2);
				}
				$error /= 2;
